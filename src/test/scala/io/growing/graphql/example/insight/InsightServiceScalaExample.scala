...
object InsightServiceScalaExample extends App {
  ...
  protected val insightHelper: ResolverImplClient.ResolverImplClientBuilder = ResolverImplClient.ResolverImplClientBuilder.newBuilder()
  ...
  println("=====tag=====")
  val tag = insightHelper.setProjection(new TagResponseProjection).setRequest(new TagQueryRequest).build(classOf[TagQueryResolver]).tag(tagId)
  println(tag)

  println("=====tags=====")
  val tags = insightHelper.setProjection(new TagResponseProjection).setRequest(new TagsQueryRequest).build(classOf[TagsQueryResolver]).tags
  println(tags)

  println("=====countTags=====")
  val countTags = insightHelper.setProjection(null).setRequest(new CountTagsQueryRequest).build(classOf[CountTagsQueryResolver]).countTags()
  println(countTags)

  println("=====deleteTag=====")
  val deleteTag = insightHelper.setProjection(null).setRequest(new DeleteTagMutationRequest).build(classOf[DeleteTagMutationResolver]).deleteTag(tagId)
  println(deleteTag)

  println("=====segments=====")
  val segments = insightHelper.setProjection(new SegmentResponseProjection).setRequest(new SegmentsQueryRequest).build(classOf[SegmentsQueryResolver]).segments()
  println(segments)

  println("=====users=====")
  val users = insightHelper.setProjection(new SlicePaginationResponseProjection).
  ...
  val createTag = insightHelper.setProjection(new TagResponseProjection).
