...
public class InsightServiceExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        ResolverImplClient.ResolverImplClientBuilder insightHelper = ResolverImplClient.ResolverImplClientBuilder.newBuilder();
        String tagId = "V0G5BZDA";

        //需要注意的是，查询一个和查询多个，它们的projection是相同的，但前者返回单个projection实体，后者返回一个集合（元素是projection）
        System.out.println("=====tag=====");
        TagDto tag = insightHelper.setProjection(new TagResponseProjection()).setRequest(new TagQueryRequest()).build(TagQueryResolver.class).tag(tagId);
        System.out.println(tag);


        System.out.println("=====tags=====");
        List<TagDto> tags = insightHelper.setProjection(new TagResponseProjection()).setRequest(new TagsQueryRequest()).build(TagsQueryResolver.class).tags();
        System.out.println(tags);
        ...
        TagDto createTag = insightHelper.setProjection(new TagResponseProjection()).
